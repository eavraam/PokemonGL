...
Model::Model(BoundTypes boundType, const char* path, float size)
    : boundType(boundType), size(size)
{
    loadModel(path);
}
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    glm::vec3 min(5000.0f);        // min point = a really LARGE, almost unreal number
    glm::vec3 max(-5000.0f);       // max point = a really SMALL, almost unreal number
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        // positions
        vector.x = mesh->mVertices[i].x * size;
        vector.y = mesh->mVertices[i].y * size;
        vector.z = mesh->mVertices[i].z * size;
    ...
    else
    {
        ...
        // We calculate the square root here so we avoid some performance cost if done earlier
